    ...
    public class SystemInfoModel(IConfiguration config) : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            while (allEnv.MoveNext())
            {
                ...
                if (key.ToLower().Contains("key", StringComparison.OrdinalIgnoreCase) || key.ToLower().Contains("secret", StringComparison.OrdinalIgnoreCase) || key.ToLower().Contains("pwd", StringComparison.OrdinalIgnoreCase) || key.ToLower().Contains("password", StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
